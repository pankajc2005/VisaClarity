...
try {
  ...
  for (const file of files) {
    ...
    for (const table of tables) {
      ...
      if (!rlsOk || !grantOk) {
        console.error(`\x1b[31m❌ DB Security Check Failed in [supabase/migrations/${file}]:\x1b[0m`);
        console.error(`   Table: "\x1b[33m${table}\x1b[0m"`);
        if (!rlsOk) {
          console.error(`   - Missing: ALTER TABLE public.${table} ENABLE ROW LEVEL SECURITY;`);
        }
        if (!grantOk) {
          console.error(`   - Missing: GRANT SELECT, INSERT, UPDATE, DELETE ON public.${table} TO authenticated;`);
        }
        failed = true;
      }
    }
