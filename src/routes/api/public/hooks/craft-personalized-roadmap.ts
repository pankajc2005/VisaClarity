...
async function processRow(row: RequestRow): Promise<void> {
  // Mark in_progress (also bumps attempts) — single-writer guard via where status=queued
  ...
  const { data: claimed, error: claimErr } = await supabaseAdmin
    ...
    .maybeSingle();
  if (claimErr || !claimed) {
    console.log("[craft-worker] row not claimable", row.id, claimErr?.message);
    return;
  }
